...
%token  SIZEOF  PTR_OP  INC_OP  DEC_OP  LEFT_OP 
%token  RIGHT_OP  ROTATE_RIGHT_OP  ROTATE_LEFT_OP  LE_OP  GE_OP 
%token  EQ_OP  NE_OP  AND_OP  OR_OP  MUL_ASSIGN 
%token  DIV_ASSIGN  MOD_ASSIGN  ADD_ASSIGN  SUB_ASSIGN  LEFT_ASSIGN 
%token  RIGHT_ASSIGN  AND_ASSIGN  XOR_ASSIGN  OR_ASSIGN  TYPEDEF_NAME 
%token  ENUMERATION_CONSTANT  TYPEDEF  EXTERN  STATIC  AUTO 
%token  REGISTER  INLINE  CONST  RESTRICT  VOLATILE 
%token  BOOL  CHAR  SHORT  INT  LONG 
%token  SIGNED  UNSIGNED  FLOAT  DOUBLE  VOID 
%token  COMPLEX  IMAGINARY  STRUCT  UNION  ENUM 
%token  ELLIPSIS  CASE  DEFAULT  IF  ELSE 
%token  SWITCH  WHILE  DO  FOR  GOTO 
%token  CONTINUE  BREAK  RETURN  ALIGNAS  ALIGNOF 
%token  ATOMIC  GENERIC  NORETURN  STATIC_ASSERT  THREAD_LOCAL 
